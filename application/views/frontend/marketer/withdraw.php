                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <select name="bank" class="form-control">
                        ...
                        ?>
                        <option value="<?= $valueBank->id; ?>"><?= $valueBank->account_name; ?> - <?= $valueBank->bank; ?></option>
                        <?php } } ?>
                      </select>
                    </div>
                  </div>
                ...
                <div class="row">
                  <div class="col-md-8">
                    <div class="form-group">
                      ...
                      <input name="nama_ktp" class="form-control" type="text" required="" plcaeholder="<?=$this->lang->line('Nama (Sesuai KTP)');?>">
                    </div>
                    <div class="form-group">
                      <label><?=$this->lang->line('No KTP');?>*</label>
                      <input name="no_ktp" class="form-control" type="number" required="" plcaeholder="<?=$this->lang->line('No KTP');?>">
                    </div>
                  </div>
                  <div class="col-md-4 margin-bottom-1x">
                    <div class="form-group">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label><?=$this->lang->line('Nama (Sesuai KTP)');?>*</label>
                      <input name="nama_ktp" class="form-control" type="text" required="" value="<?= $checkBank[0]->nama_ktp; ?>" placeholder="<?=$this->lang->line('Nama (Sesuai KTP)');?>">
                    </div>
                    <div class="form-group">
                      <label><?=$this->lang->line('No KTP');?>*</label>
      ...
      <?php if (!$this->input->get('addbank') && !$this->input->get('editbank') && !$this->input->get('payout')) { ?>
        <div class="row">
          <div class="col-lg-8">
                ...
                $payout=0; // variable ini termasuk bagian validasi apakah user dapat request withdraw kembali atau tidak
                if (count($withdraw) > 0) {
                ?>
                <table class="table table-hover margin-bottom-none">
                  ...
                  <?php foreach ($withdraw as $key => $valueWithdraw) {
                    $payout=$payout+$valueWithdraw->payout;
                  ?>
                    <tr>
                      <td><?= $key + 1; ?></td>
                ...
                <?php } else {
                  echo "<div class='padding-bottom-2x text-center'>".$this->lang->line('Tidak ada riwayat widthdrawal')."</div><hr class='margin-bottom-2x'>";
                } ?>
                ...
                <table class="table table-hover margin-bottom-none">
                  ...
                  <?php foreach ($bank as $key => $valueBank) { ?>
                    <tr>
                      ...
                      <td>
                        <a href="<?= site_url('marketer/withdraw?editbank=').$valueBank->id; ?>" style="text-decoration: none;" class="badge badge-primary badge-pill"><?=$this->lang->line('Edit');?></a>
                      </td>
                    </tr>
                  <?php } ?>
                  </tbody>
              ...
              <?php } else { ?>
                ...
                <a href="<?= site_url('marketer/withdraw?addbank=true'); ?>" class="btn btn-info"><?=$this->lang->line('Add Bank Account');?></a>
            ...
            <div class="col-lg-4 text-center">
              ...
              <?php if ($user_data->wallet >= 250000 ) { ?>
                <a href="<?= site_url('marketer/withdraw?payout=true'); ?>" class="btn btn-success"><?=$this->lang->line('Request Withdrawal');?></a>
              <?php } ?>
            </div>
          </div>
        </div>

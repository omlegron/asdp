...
<?php
  ...
  if (count($checkStore) > 0) {
    $sql_ordersStore="select A.*, SUM(A.quantity) as sum_quantity
                      from orders_detail A 
                      join orders B on (A.orders = B.id)
                      join orders_store C on (C.orders = B.id)
                      where MONTH(B.created) = '".$month_query."' && YEAR(B.created) = '".$year."' && C.store='".$checkStore[0]->id."'  && C.orders_status=3
    ...
    $orderDetail = $this->m_model->selectcustom($sql_ordersStore);
    ?>
    ...
    if (count($orderDetail) > 0) {
      ...
      foreach ($orderDetail as $key => $valueDetail) {
        $subtotal = 0;
        $subtotal += $valueDetail->price * $valueDetail->sum_quantity;
        $grand_total+=$subtotal;

      $product_store = $this->m_model->selectas('id', $valueDetail->product_store, 'product_store');
  ...
  ?>
      <tr>
        ...
        <td class="text-center"><?= number_format($valueDetail->sum_quantity); ?></td>
        <td class="text-right"><?= 'Rp '.number_format($valueDetail->price); ?></td>
...
?>
         <tr>
          <th class="text-right" colspan="4" style="border-top:1px solid #333;"><?=$this->lang->line('Total');?></th>

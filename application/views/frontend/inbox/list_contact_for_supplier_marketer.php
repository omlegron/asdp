...
<?php
	...
	$getContact = $this->m_model->selectasgroup('receiver_id', $user_data->id, 'sender_id', 'chats', 'id', 'DESC');
?>
<div class="inbox_people">
  	<div class="headind_srch">
    	<div class="recent_heading">
     		<h4><?=$this->lang->line('Recent');?></h4>
    	</div>
    	<!--<div class="srch_bar">
    		...
    		<?php
    			if(count($getContact)>0){	
	    			foreach ($getContact as $key => $value) {
	    				# code...
	    				//get user_data
						$getUser = $this->m_model->selectas('id', $value->sender_id, 'user');
						$last_message="";
						if($value->sts !=3){
						...
						if(count($getUser)>0){
							...
							foreach ($getUser as $key => $valueUser) {
								$getStore = $this->m_model->selectas('user', $value->sender_id, 'store');
								# code...
								$imgReceiver="";
